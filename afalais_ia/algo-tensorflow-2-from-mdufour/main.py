...
def generate_full_dataset():
    ...
    for directory in dirs:
        ...
        try:
            ...
            print(
                'Generate full dataset with xray images... Please wait it can take a while.')
        ...
        with os.scandir('chest_xray/{}/PNEUMONIA'.format(dtype)) as entries:
            for entry in entries:
                if entry.name.find('virus') > -1:  # this is a virus
                    ...
                    shutil.copy2(
                        entry.path, 'chest_xray/{}/VIRUS/'.format(dtype))
                else:
                    print('Move bacteria img.')
                    shutil.copy2(
                        entry.path, 'chest_xray/{}/BACTERIA/'.format(dtype))
...
def main():
    ...
    if (os.path.isfile('saved_model/{}/model.h5'.format(MODEL_NAME))):
        model = tf.keras.models.load_model(
            'saved_model/{}/model.h5'.format(MODEL_NAME))
        modelLoaded = True
